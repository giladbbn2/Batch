...
{
    public class WorkerLoader : MarshalByRefObject, IDisposable
    {
        ...
        public static ConcurrentDictionary<string, WorkerLoader> AppDomainToWorkerLoader;
        public static ConcurrentDictionary<string, WorkerLoader> AssemblyPathToWorkerLoader;
        ...
        public static void Init()
        {
            if (!isInit)
                lock (wlsync)
                {
                    if (!isInit)
                    {
                        AppDomainToWorkerLoader = new ConcurrentDictionary<string, WorkerLoader>();
                        AssemblyPathToWorkerLoader = new ConcurrentDictionary<string, WorkerLoader>();
                        isInit = true;
                    }
                }
        }

        public static WorkerLoader RegisterInstance(string AppDomainName, string PathToAssembly)
        {
            if (!isInit)
                Init();
            ...
            WorkerLoader wl, wlAppDomain, wlPath;
            ...
            else
            {
                ...
                wl = (WorkerLoader)ad.CreateInstanceAndUnwrap(typeof(WorkerLoader).Assembly.FullName, typeof(WorkerLoader).FullName);
        ...
        public static bool TryGetInstanceByAppDomainName(string AppDomainName, out WorkerLoader wl)
        {
            return AppDomainToWorkerLoader.TryGetValue(AppDomainName, out wl);
        }

        public static bool TryGetInstanceByPathToAssembly(string PathToAssembly, out WorkerLoader wl)
        {
        ...
        public static void UnloadByAppDomainName(string AppDomainName)
        {
            ...
            WorkerLoader wlAppDomain, wlPath;
